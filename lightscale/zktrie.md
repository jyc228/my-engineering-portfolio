...
저에게 주어진 초기 과제는, 새로운 서버가 최신 데이터를 빠르게 따라잡고 디스크 사용량을 절반으로 줄여주는 [스냅싱크](./snapsync.md)라는 중요 기능 구현이었습니다. 하지만 분석 과정에서 저는 이 기능이 의존하는 코어 데이터 구조의 처리 방식이 장기적인 안정성에 큰 위험이 될 수 있음을 발견했습니다.
...
저는 기능 하나를 구현하는게 아닌 조직이 운영하는 블록체인의 미래를 예상했습니다. 그러니 전면 재구현이 필수라고 판단했고, 지속적으로 코어 트리의 성능 문제를 예견하며 팀을 설득했습니다. 그 결과 프로젝트는 전면 재구현으로 방향이 설정되었지만, 초기에는 리스크를 최소화하기 위해 상대적으로 안전한 스냅싱크 기능에서만 사용하는 것으로 결정했습니다.
...
팀원 모두가 정확한 원인을 파악하지 못하는 상황에서, 저는 **이전까진 문제 없다가 tps 가 올라간 뒤 장애가 발생했다** 라는 사실을 기반으로 zktrie 의 성능 문제로 추측했습니다.
저는 트리 성능 이슈 라고 주장을 했고, 받아들여져서 인스턴스 스펙 증가로 대응했으며 이후 체인은 안정화 되었으나 미봉책에 불과했습니다. 제가 구현중인 트리가 근본적인 해결책이었기 때문에 마무리 단계에 있던 트리를 구현 완료하고
