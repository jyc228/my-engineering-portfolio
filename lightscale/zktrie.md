...
저에게 주어진 초기 과제는, 새로운 서버가 최신 데이터를 빠르게 따라잡고 디스크 사용량을 절반으로 줄여주는 [스냅싱크](./snapsync.md)라는 중요 기능 구현이었습니다.
하지만 분석 과정에서 저는 이 기능이 의존하는 코어 데이터 구조의 처리 방식이 장기적인 안정성에 큰 위험이 될 수 있음을 발견했습니다.

당시 제게 주어진 공식적인 업무는 **스냅싱크 구현**이었지, **코어 트리 재구현**이 아니었습니다. 이 상황에서 두 가지 선택지를 두고 고민했습니다.
하나는 주어진 과제인 스냅싱크의 목표 달성에만 집중하는 맞춤형 구현이었고, 다른 하나는 시스템 전체에 적용될 범용적인 코어 트리의 전면 재구현이었습니다.

조직이 운영하는 블록체인의 미래를 예상해보니 전면 재구현이 필수라고 판단했고, 코어 트리의 성능 문제를 예견하며 팀을 설득했습니다.
그 결과 프로젝트는 전면 재구현으로 방향이 설정되었지만, 초기에는 리스크를 최소화하기 위해 상대적으로 안전한 스냅싱크 기능에서만 사용하는 것으로 결정했습니다.
...
2024년 1월경, 기존 시스템은 tps 10을 견디지 못하고 체인이 약 6시간 동안 중단되는 심각한 사태가 발생했습니다.
당시 팀 내에서는 장애의 원인이 파악되지 않았으나, 저는 시스템의 TPS 부하 패턴과 트리 자료구조의 특성을 분석하여 **트리 성능 이슈** 로 장애 원인을 진단했습니다.

우선 서비스 복구를 위해 인스턴스 스펙을 상향 조정하는 즉각적인 완화책을 실행했습니다.
하지만 이는 일시적인 조치임을 인지하고, 마무리 단계에 있던 트리를 구현 완료하고 내부 검증, 외부 업체 코드 감사 후 서비스에 적용하여 근본적인 문제를 해결했습니다.
...
당시의 치열했던 고민과 기술적 분석을 담은 **저희 팀의 공식 사후 분석 보고서(Post-Mortem)** 는 아래 링크에서 확인하시거나, 링크 유실을 대비하여 제일 아래에 별도 첨부 하였습니다.
...
ensure proper functionality. The ZKTrie has also been audited
by [Chainlight](https://github.com/kroma-network/go-ethereum/blob/main/docs/audits/2024-02-23_KromaZKTrie_Security_Audit_ChainLight.pdf).
