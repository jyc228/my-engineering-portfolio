...
이더리움 P2P 동기화 프로토콜(`스냅싱크`)을 자체 기술 스택에 맞게 직접 재구현 → 신규 노드 동기화 시간 2주 이상에서 2일 이내로 단축

## 배경 & 도전

신규 노드가 네트워크에 참여할 때 전체 이력을 검증하는 대신 최신 상태만 동기화하는 핵심 기능(`스냅싱크`)이 자체 기술 스택과의 비호환으로 동작하지 않고 있었습니다.

이를 재활성화하는 작업을 맡았으나, 핵심 자료구조와 강결합된 구조 때문에 단순 수정이 아닌 프로토콜 레이어 전체를 재구현해야 했습니다.
...
- 기술 내재화: `geth` 프로토콜의 하위 레이어를 직접 제어함으로써 팀 내 블록체인 코어 기술 자립도를 확보했습니다.
